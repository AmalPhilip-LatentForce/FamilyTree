...
const GedcomUpload = ({ onDataParsed }) => {
  ...
  const parseGEDCOM = (content) => {
    ...
    lines.forEach((line) => {
      ...
      if (levelNum === 0) {
        ...
        if (type === "INDI") {
          currentType = "INDI";
          currentEntity = id.replace(/@/g, "");
          // Initialize person with all possible fields
          persons[currentEntity] = {
            id: currentEntity,
            name: "Unknown",
            birthyear: null,
          ...
          unions[currentEntity] = {
            ...
            marriage_date: null,
            marriage_place: "",
            isVisible: true  // Add visibility flag for unions
          };
        } else {
          currentType = null;
          currentEntity = null;
        ...
        if (currentType === "INDI") {
          switch (tag) {
            case "NAME":
              persons[currentEntity].name = value.replace(/\//g, "").trim() || "Unknown";
              break;
            case "SEX":
              persons[currentEntity].gender = value.trim();
              break;
            ...
            case "DATE":
              if (currentSubTag === "BIRT") {
                const yearMatch = value.match(/\d{4}/);
                persons[currentEntity].birthyear = yearMatch ? parseInt(yearMatch[0]) : null;
              } else if (currentSubTag === "DEAT") {
                const yearMatch = value.match(/\d{4}/);
                persons[currentEntity].deathyear = yearMatch ? parseInt(yearMatch[0]) : null;
              }
              break;
            case "PLAC":
              if (currentSubTag === "BIRT") {
          ...
          switch (tag) {
            ...
            case "DATE":
              if (currentSubTag === "MARR") {
                const yearMatch = value.match(/\d{4}/);
                unions[currentEntity].marriage_date = yearMatch ? parseInt(yearMatch[0]) : null;
              }
              break;
            case "PLAC":
              if (currentSubTag === "MARR") {
                unions[currentEntity].marriage_place = value.trim();
              }
              break;
            default:
              break;
    ...
    // Function to sort persons by generations while maintaining visibility
    const sortPersonsByGenerations = (persons, unions) => {
      const getGeneration = (personId, cache = new Map()) => {
        if (cache.has(personId)) return cache.get(personId);
    ...
    // Use fixed start point as in first version
    const start = "I1";
  
    // Ensure all nodes are marked as visible
    Object.values(sortedPersons).forEach(person => {
      person.isVisible = true;
    });
    Object.values(unions).forEach(union => {
      union.isVisible = true;
    });
  ...
  const addSpouseToGedcom = (
    ...
    // Create new individual entry for spouse
    const spouseEntry = [
      `0 @${spouseData.id}@ INDI`,
      `1 NAME ${spouseData.name}`,
      `1 SEX ${spouseData.gender}`,
      `1 BIRT`,
      `2 DATE ${spouseData.birthyear}`,
      `2 PLAC ${spouseData.birthplace}`,
      spouseData.deathplace ? `1 DEAT\n2 PLAC ${spouseData.deathplace}` : "",
      `1 FAMS @${newFamId}@`,
    ].filter((line) => line !== "");
    ...
    const familyEntry = [
      `0 @${newFamId}@ FAM`,
      personData.gender === "M"
        ? `1 HUSB @${personData.id}@\n1 WIFE @${spouseData.id}@`
        : `1 HUSB @${spouseData.id}@\n1 WIFE @${personData.id}@`,
    ];
    ...
    // Update existing person's record to include new family reference
    const personIndex = lines.findIndex((line) =>
      line.includes(`@${personData.id}@ INDI`)
    );
    if (personIndex !== -1) {
      // Find the end of the person's record
      let i = personIndex + 1;
      while (i < lines.length && !lines[i].startsWith("0")) {
        i++;
      }
      // Add FAMS reference
      lines.splice(i, 0, `1 FAMS @${newFamId}@`);
    }
    ...
    if (firstFamIndex !== -1) {
      // Insert family entry before other FAM records
      lines.splice(firstFamIndex, 0, ...familyEntry, "");
      // Insert spouse entry after last INDI record
      lines.splice(lastIndiIndex + 1, 0, ...spouseEntry, "");
    } else {
      // Add to end if no FAM records exist
      lines.push(...spouseEntry, "", ...familyEntry);
    }

    return lines.join("\n");
  };

  const handleAddSpouse = (spouseData, personData) => {
    try {
      // Update GEDCOM content
      const updatedContent = addSpouseToGedcom(
        gedcomContent,
        spouseData,
        personData,
        parsedData.unions
      ...
      const updatedData = {
        ...
        persons: {
          ...
          [personData.id]: {
            ...personData,
            own_unions: [...(personData.own_unions || []), newFamId],
          },
        },
        unions: {
          ...parsedData.unions,
          [newFamId]: {
            id: newFamId,
            partner: [
              personData.gender === "M" ? personData.id : spouseData.id,
              personData.gender === "M" ? spouseData.id : personData.id,
            ],
            children: [],
          },
        },
        links: [
          ...parsedData.links,
          [personData.id, newFamId],
          [spouseData.id, newFamId],
        ],
      };
