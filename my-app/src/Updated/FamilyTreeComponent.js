...
const FamilyTreeComponent = ({
  ...
  useEffect(() => {
    ...
    const handlePersonSelected = (event) => {
      const selectedPerson = event.detail;
      onPersonSelect(selectedPerson);
    };

    window.addEventListener("personSelected", handlePersonSelected);

    return () => {
      window.removeEventListener("personSelected", handlePersonSelected);
    };
  }, [onPersonSelect]);
  ...
  useEffect(() => {
    ...
    const initializeFamilyTree = () => {
      ...
      try {
        ...
        // Store FT instance globally for external access
        window.currentFamilyTree = FT;
      } catch (error) {
        console.error("Error initializing family tree:", error);
      }
    };
    ...
    return () => {
      clearInterval(scriptCheckInterval);
      delete window.familyTreeData;
      delete window.currentFamilyTree;
      delete window.selectedFamilyMember;
    };
  }, [data, width, height]);
