...
//         node.id = node.id || this.number_nodes;
//         this.number_nodes++;
//         // Set all nodes visible by default
//         node.visible = true;
...
//       // Update roots to show full tree
//       this.update_roots();
//     }
//     else if (Object.keys(data.persons).length > 0) {
//       const root_data = data.persons[start_node_id];
...
//   update_roots() {
//     // Modified to ensure all nodes are included
//     this.dag.children = this.nodes.filter(node =>
//       node instanceof Person && (!node.get_parent_unions().length ||
//       node.get_parent_unions().every(union => !this.nodes.includes(union)))
//     );
//   }
...
//   show() {
//     // Modified to show all connected nodes immediately
...
//     this.get_parent_unions().forEach((union) => {
//       union.visible = true;
//       union.show();
//       this.inserted_nodes.push(union);
//     });
