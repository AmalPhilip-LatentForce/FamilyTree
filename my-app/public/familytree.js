// *********************************************************************** code with color gender and rectangular box functionaltiy
...
//     this.data = data; // Store the full data object
...
//     super(dagNode.id, ft_datahandler.data.unions[dagNode.id]);
//     // link to new object
//     dagNode.ftnode = this;
...
// console.log("Setting FamilyTree on window object...");
// window.FamilyTree = FamilyTree;
// console.log("FamilyTree script loaded successfully");

// *********************************************************************************** code without tranistion and mouse click node functionality
...
class FTDrawer {
  ...
  selectNode(node) {
    if (!node.is_union()) {
      ...
      // Store the selected person data globally
      window.selectedFamilyMember = this.ft_datahandler.data.persons[node.id];
      ...
      const event = new CustomEvent("personSelected", {
        detail: window.selectedFamilyMember,
      });
  ...
  draw(source = this.ft_datahandler.root) {
    ...
    var nodeEnter = node
      ...
      .on("click", (event, node) => {
        ...
        // Handle both selection and expansion/collapse
    ...
    nodeEnter
      .append("rect")
      .attr("class", (d) => {
        const baseClass = this.node_class_func(d);
        return baseClass + (d.id === this.selectedNode?.id ? " selected" : "");
      })
      .attr("data-id", (d) => d.id)
      .attr("width", 100)
      .attr("height", 50)
      ...
      .attr("ry", 5)
      .style("stroke", (d) =>
        d.id === this.selectedNode?.id ? "#2196F3" : "#000"
      )
      .style("stroke-width", (d) =>
        d.id === this.selectedNode?.id ? "3px" : "1px"
      );
...
class FamilyTree extends FTDrawer {
  constructor(data, svg) {
    ...
    window.selectedFamilyMember = null;

    // Add listener for external selection changes
    window.addEventListener("familyMemberSelectedFromList", (event) => {
      const personId = event.detail.id;
      const node = this.ft_datahandler.find_node_by_id(personId);
      if (node) {
        this.selectNode(node);
      }
    });
